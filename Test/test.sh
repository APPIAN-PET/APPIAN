...
docker run -v "$SCRIPTPATH":"/APPIAN" tffunck/appian:latest bash -c "python2.7 /APPIAN/Launcher.py  --user-brainmas --user-t1mni --threads 3   --pvc-label-img "cls"   --tka-method 'lp' --tka-label-img "cls" --tka-labels-brain-only --tka-labels-ones-only  --tka-label 3 --tka-label-erosion 5  --results-labels-brain-only --results-label 2 --results-label-img "cls" --sourcedir /APPIAN/Test/test_data_fmz --targetdir /APPIAN/Test/out_test_fmz  --tasks 1 --pet-scanner HRRT  01";

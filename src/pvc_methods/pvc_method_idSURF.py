...
class pvcInput(CommandLineInputSpec):
    ...
    nvoxel_to_src = traits.Int( argstr="-nvoxel-to-src %f", default_value=64, usedefault=True, desc="Number of voxels to src over.")
...
def check_options(pvcNode, opts):
    ...
    if opts.nvoxel_to_src != None : pvcNode.inputs.nvoxel_to_src=opts.nvoxel_to_src
    return pvcNode

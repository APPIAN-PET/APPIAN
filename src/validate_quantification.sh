...
python3 ${appian_path}/Launcher.py  -s $dir -t $out --subjects $subs --sessions $sess  --no-qc --user-ants-command ${appian_path}/src/ants_command_quick.txt
for method in $methods ; do
    for kind in quant-roi ; do #quant-voxel 
        ###
        ### RUN APPIAN
        ###
